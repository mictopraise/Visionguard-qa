...
def test_isolated_motion_spike_is_review_only() -> None:
    result = confirm_issue(_issue("motion_discontinuity", 5.0, 5.0, 1.0, raw_events=1))
    assert result["status"] == "review"
    assert result["confirmed"] is False


def test_persistent_motion_cluster_is_confirmed() -> None:
    result = confirm_issue(_issue("motion_discontinuity", 5.0, 5.5, 1.0, raw_events=6))
    assert result["status"] == "confirmed"
    assert result["confirmed"] is True
